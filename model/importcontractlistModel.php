...
Class importcontractlistModel Extends baseModel {
	protected $table = "import_contract_list";
    ...
    public function deleteImport($id){
    	if ($this->getImport($id)) {
    		return $this->delete($this->table,array('import_contract_list_id'=>$id));
    	}
    }
    ...
    public function getAllImportByWhere($id){
        return $this->query('SELECT * FROM import_contract_list WHERE import_contract_list_id != '.$id);
    }
    public function getLastImport(){
        return $this->getLast($this->table);
    }
    public function queryImport($sql){
