...
Class tirepricediscountnumberController Extends baseController {
    ...
    public function index() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $order_by = $this->registry->router->order_by ? $this->registry->router->order_by : 'type ASC, start_date';

            $order = $this->registry->router->order ? $this->registry->router->order : 'DESC, tire_price_discount_number_id ASC';
        ...
        $tire_price_discount_model = $this->model->get('tirepricediscountnumberModel');
        ...
        $tongsodong = count($tire_price_discount_model->getAllTire());
        ...
        $this->view->data['tires'] = $tire_price_discount_model->getAllTire($data);



        $this->view->data['lastID'] = isset($tire_price_discount_model->getLastTire()->tire_price_discount_number_id)?$tire_price_discount_model->getLastTire()->tire_price_discount_number_id:0;

        

        $this->view->show('tirepricediscountnumber/index');
    ...
    public function add(){
        ...
        if (isset($_POST['yes'])) {
            ...
            if ($_POST['yes'] != "") {
                    ...
                    $tire1 = $tire_price_discount_model->getTireByWhere(array('type'=>$tire_d->type,'number'=>$tire_d->number,'end_date'=>(strtotime(date('d-m-Y',strtotime(date('d-m-Y',$tire_d->start_date).' -1 day'))))));
                    $tire2 = $tire_price_discount_model->getTireByWhere(array('type'=>$tire_d->type,'number'=>$tire_d->number,'start_date'=>(strtotime(date('d-m-Y',strtotime(date('d-m-Y',$tire_d->end_date).' +1 day'))))));
                    if($tire1)
                        $tire_price_discount_model->updateTire(array('type'=>$tire_d->type,'number'=>$tire_d->number,'end_date'=>(strtotime(date('d-m-Y',strtotime($_POST['start_date'].' -1 day'))))),array('tire_price_discount_number_id' => $tire1->tire_price_discount_number_id));
                    if($tire2)
                        $tire_price_discount_model->updateTire(array('type'=>$tire_d->type,'number'=>$tire_d->number,'start_date'=>(strtotime(date('d-m-Y',strtotime($_POST['end_date'].' +1 day'))))),array('tire_price_discount_number_id' => $tire2->tire_price_discount_number_id));


                    $tire_price_discount_model->updateTire($data,array('tire_price_discount_number_id' => trim($_POST['yes'])));
                    ...
                    date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        ...
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."edit"."|".$_POST['yes']."|tire_price_discount_number|".implode("-",$data)."\n"."\r\n";
                ...
                if ($tire_price_discount_model->getTireByWhere(array('type'=>$data['type'],'number'=>$data['number'],'start_date'=>$data['start_date'],'end_date'=>$data['end_date']))) {
                    ...
                    $dm1 = $tire_price_discount_model->queryTire('SELECT * FROM tire_price_discount_number WHERE type='.$data['type'].' AND number='.$data['number'].' AND start_date <= '.$data['start_date'].' AND end_date <= '.$data['end_date'].' AND end_date >= '.$data['start_date'].' ORDER BY end_date ASC LIMIT 1');
                    $dm2 = $tire_price_discount_model->queryTire('SELECT * FROM tire_price_discount_number WHERE type='.$data['type'].' AND number='.$data['number'].' AND end_date >= '.$data['end_date'].' AND start_date >= '.$data['start_date'].' AND start_date <= '.$data['end_date'].' ORDER BY end_date ASC LIMIT 1');
                    $dm3 = $tire_price_discount_model->queryTire('SELECT * FROM tire_price_discount_number WHERE type='.$data['type'].' AND number='.$data['number'].' AND start_date <= '.$data['start_date'].' AND end_date >= '.$data['end_date'].' ORDER BY end_date ASC LIMIT 1');
                    ...
                    if ($dm3) {
                            foreach ($dm3 as $row) {
                                ...
                                $tire_price_discount_model->updateTire($d,array('tire_price_discount_number_id'=>$row->tire_price_discount_number_id));
                    ...
                    date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        ...
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."add"."|".$tire_price_discount_model->getLastTire()->tire_price_discount_number_id."|tire_price_discount_number|".implode("-",$data)."\n"."\r\n";
            ...
            if (isset($_POST['xoa'])) {
                ...
                foreach ($data as $data) {
                    ...
                    date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        ...
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."delete"."|".$data."|tire_price_discount_number|"."\n"."\r\n";
                ...
                date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        ...
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."delete"."|".$_POST['data']."|tire_price_discount_number|"."\n"."\r\n";
