...
Class checksalarytotalController Extends baseController {
    public function index() {
        ...
        $qr = 'SELECT * FROM (SELECT * FROM time_salary WHERE create_time <= '.strtotime($ketthuc).' ORDER BY create_time DESC) d GROUP BY d.staff';
        $time_salarys = $time_salary_model->querySalary($qr);
        $arr_time_salary = array();
        foreach ($time_salarys as $time_salary) {
            $arr_time_salary[$time_salary->staff] = isset($arr_time_salary[$time_salary->staff])?$arr_time_salary[$time_salary->staff]+$time_salary->time_salary:$time_salary->time_salary;
        }

        $this->view->data['arr_time_salary'] = $arr_time_salary;

        $data = array(
            'where' => 'create_time >= '.strtotime($batdau).' AND create_time <= '.strtotime($ketthuc),
        );

        $overtime_salarys = $overtime_salary_model->getAllSalary($data);
        $arr_overtime_salary = array();
        foreach ($overtime_salarys as $overtime_salary) {
            $arr_overtime_salary[$overtime_salary->staff] = isset($arr_overtime_salary[$overtime_salary->staff])?$arr_overtime_salary[$overtime_salary->staff]+$overtime_salary->overtime_salary:$overtime_salary->overtime_salary;
        }
    ...
    public function index3() {
        ...
        /*************/
        $this->view->show('checksalarytotal/index3');
    }
    ...
    public function ngoaigio(){
        ...
        if (isset($_POST['data'])) {
            ...
            $overtime_salary = $this->model->get('overtimesalaryModel');

            $overtime_salary_data = $overtime_salary->getAllSalary(array('where'=>'create_time >= '.strtotime($batdau).' AND create_time <= '.strtotime($ketthuc).' AND staff = '.$_POST['data']));

            $data = array(
                'staff' => $_POST['data'],
                'create_time' => strtotime($batdau),
                'overtime_salary' => trim(str_replace(',','',$_POST['keyword'])),
                );

            if (!$overtime_salary_data) {
                $overtime_salary->createSalary($data);
            }
            else{
                $overtime_salary->querySalary('UPDATE overtime_salary SET overtime_salary = '.trim(str_replace(',','',$_POST['keyword'])).' WHERE staff = '.$_POST['data'].' AND create_time >= '.strtotime($batdau).' AND create_time <= '.strtotime($ketthuc));
